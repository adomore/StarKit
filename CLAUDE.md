...
**Phase 1. Gate G0 closed 2026-07-16** — T0-1…T0-4 AC green (`./ci.sh`), Q1 answered (D-026: Windows + standalone GUI), Q2 deferred (D-027), Q3/Q4/Q6 deferred (D-028). **INV-5 is discharged: product code may now be written.** Next task: **T1-7** (reduction A, resynthesis — the default, and the one FR-5's full AC grades). T1-1…T1-6 done: `starkit-io` round-trips TIFF16 pixel-identically; `background` 0.19 ADU RMS; `detect` meets all four FR-2 bars on `basic-5k` (99.94 % / 99.92 % / 0.098 px / 5.77 %); `mask` exports 16-bit tiered masks with pinned golden hashes; `gate` is INV-1's single enforcement point — proved on `nightscape-fg` at zero tolerance, D-034's debt paid (precision 35.7 % → 99.91 %); `reduce::morphological` is method B, its outside-gate bit-identity proved on `basic-5k` and `nightscape-fg`. **Every star operation composites only through `gate::composite`** — a direct write into an output plane anywhere else is a review blocker. Standing debt: **G1 cannot close without the real corpus** (D-027) — FR-2's real-image AC, FR-4's photographer sign-off and T1-10 are all blocked on it.
